...
void dijkstra(Graph g, int source)
{
    ...
    while (queue.size())
    {
        ...
        printf("\nRemoveu nó %d da queue", minimum);
     
        // foreach uv from that connects the choosen node
        // we update the distance (relaxation process)
        for (auto it:g.adj[minimum])
        {
            printf("\nu:%d, d(u):%d", minimum, dist[minimum]);
            printf("\nv:%d, d(v):%d, l(u,v)=%d", it.first, dist[it.first], it.second);
            printf("\n----");
            printf("%d;%d", dist[it.first], dist[minimum] + it.second);
